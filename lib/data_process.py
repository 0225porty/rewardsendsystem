...
def create_base_path(params):
    ...
    # チェックファイルの取得
    checkfilename = common.check_file[params['mode']]
    shutil.copy(os.path.join(myEnv['log_path'],checkfilename),myEnv['export_path'])
...
def output_log(params,level,msg):
    ...
    if level == 0:
        ws.cell(row=2,column=3).value = params['target']
        ws.cell(row=3,column=3).value = params['sent_type']
        ws.cell(row=4,column=3).value = params['reward_type']
        ws.cell(row=5,column=3).value = params['campaign_id']
        ws.cell(row=6,column=3).value = dt.datetime.now()
    elif level == 1:
...
def set_params(params,df:pd):
    ...
    # 他社特典の場合の処理
    if str(params['reward_type']) != 'プロモコード':
        col_cnt = df.shape[1]
    ...
    # ローソンお買物券の場合の処理
    if str(params['reward_type']) == '他社特典／ローソンお買物券':
        df['lawson_code'] = f'発券コード：{df['column1']} 発券キー：{df['column2']}'
        output_log(params,1,f"ローソンお買物券のコードを設定")
...
def format_data(params,edit_rule,df:pd.DataFrame):
    ...
    # 25/8/22時点では編集するロジックは不要のため記載していない
    # 編集ロジックを追加する場合は、「edit_rule」変数を使用

    # [MaxLimit]が1以上の場合は、特典に回数分の文字を付加する
    # [benefit for:]の文字を削除
...
def add_maxlimit(params,df:pd.DataFrame):
    ...
    output_log(params,1,f"特典名の成型完了")
